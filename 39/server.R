library(shiny)
library(XML)
data <- read.table("data_39_20180620.txt",header = TRUE,stringsAsFactors = FALSE)
dataname <- c("英雄榜","昵称","大区","服务器","等级","门派","势力","装评","修为","力","魂","体","敏","疾","念","最小物攻","最大物攻","最小法攻",
              ...
              "戒指二","裤子","项链","玉佩","鞋","启慧","天灵")


shinyServer(function(input, output) {
  paixu <- reactive({
    validate(
      need(as.numeric(input$value) > 26000,
           message  = "请在装评上限输入26000以上的数字")
    )
